...
const createBookingSchema = z.object({
  ...
  fullname: z.string().min(2, 'Full name must be at least 2 characters').max(200, 'Full name too long'),
...
const updateBookingSchema = z.object({
  fullname: z.string().min(2, 'Full name must be at least 2 characters').max(200, 'Full name too long').optional(),
...
router.get('/:id', validateRequest({ params: bookingParamsSchema }), async (req, res, next) => {
  try {
    const { id } = req.params
...
router.post('/', validateRequest({ body: createBookingSchema }), async (req, res, next) => {
  try {
...
router.post('/round-trip', validateRequest({ body: createRoundTripBookingSchema }), async (req, res, next) => {
  try {
    const { outbound_flight_id, return_flight_id, fullname, email, phone } = req.body

    // Verify both flights exist
    const outboundFlight = await db
      .select()
      .from(flights)
      .where(and(eq(flights.id, outbound_flight_id), isNull(flights.deleted_at)))
      .limit(1)

    const returnFlight = await db
      .select()
      .from(flights)
      .where(and(eq(flights.id, return_flight_id), isNull(flights.deleted_at)))
      .limit(1)
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const totalPrice = Number(outboundFlight[0].price) + Number(returnFlight[0].price)
...
router.delete('/:id', validateRequest({ params: bookingParamsSchema }), async (req, res, next) => {
  try {
    const { id } = req.params
